...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        return tree(total_weight(m), [totals_tree(end(m[i])) for i in range(1,3)])
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount, input_password):
        nonlocal wrong_passwords, amount_left
        if len(wrong_passwords) == 3:
            return "Your account is locked. Attempts: ['" + wrong_passwords[0] + "', '" + wrong_passwords[1] + "', '" + wrong_passwords[2] + "']"
