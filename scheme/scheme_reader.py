...
class Pair(object):
    ...
    def __getitem__(self, index):
        if self == nil:
            ...
            return self.first
        return Pair.__getitem__(self.second, index - 1)
...
def scheme_read(src):
    ...
    if val == 'nil':
        return nil # If it reads, return nil object
    elif val == '(':
        return read_tail(src) # If it is a call expression, read the inner part
    elif val in quotes:
        if val == "'":
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
        ...
        elif src.current() == ')': # Indicates when the call expression finishes
            src.remove_front()
            return nil
        elif src.current() == '.':
            src.remove_front()
            ...
            if src.current() == ')':
                ...
                raise SyntaxError('Only one element must follow a dot')
        else: 
