...
def eval_all(expressions, env):
    ...
    if expressions == nil:
        return None
    all_expr = expressions.map(lambda x: scheme_eval(x, env))
    return all_expr[len(all_expr) - 1]
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('wrong number of arguemnts are passed to function')
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        check_form(curr_binding, 2)
        symbol, val = curr_binding.first, scheme_eval(curr_binding.second.first, env)
