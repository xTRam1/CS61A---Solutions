...
def about(topic):
    ...
    def select(paragraph):
        for word in topic:
            if lower(word) in lower(paragraph):
                return True
        return False
    return select
    # END PROBLEM 2
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    num_typed = len(typed) / 5
    minutes = 60 / elapsed
    return minutes * num_typed
    # END PROBLEM 4
...
def edit_diff(start, goal, limit):
    ...
    if goal == "" or start == "": 
        ...
        add_diff = edit_diff(goal[0] + start, goal[1:], limit)
        remove_diff = edit_diff(start[:1], goal[:1], limit)
        substitute_diff = edit_diff(goal[0] + start[:1], goal[1:], limit)
